class OrbitCamera extends pc.ScriptType {
	...
	public distanceBetween = new pc.Vec3();
	public pitchAngleMin;
	public pitchAngleMax;
	public distanceMin;
	public distanceMax;
	public inertiaFactor;
	public focusEntity;
	...
	public set distance(value) {
		this._targetDistance = this._clampDistance(value + this.zoomOffset);
	}
	...
	public set pivotPoint(value) {
		this._pivotPoint.copy(value);
	}

	// Moves the camera to look at an entity and all its children so they are all in the view
	public focus(focusEntity) {
	...
	public resetAndLookAtPoint(resetPoint, lookAtPoint) {
	...
	public forceLook(rotation){
		let yaw = this._calcYaw(rotation);

		this.yaw = this._calcYaw(rotation);
		this.pitch = this._calcPitch(rotation, yaw);
	}

	// Set camera position to a world position and look at an entity in the scene
	// Useful if you have multiple models to swap between in a scene
	public resetAndLookAtEntity(resetPoint, entity) {
		this._buildAabb(entity, 0);
		this.resetAndLookAtPoint(resetPoint, this._modelsAabb.center);
	}

	// Set the camera at a specific, yaw, pitch and distance without inertia (instant cut)
	public reset(yaw, pitch, distance) {
		this.pitch = pitch;
		this.yaw = yaw;
		this.distance = distance;
	...
	private _buildAabb(entity, modelsAdded) {
		var i = 0;
	...
	private _clampDistance(distance) {
	...
	private _clampPitchAngle(pitch) {
		// Negative due as the pitch is inversed since the camera is orbiting the entity
		return pc.math.clamp(pitch, -this.pitchAngleMax, -this.pitchAngleMin);
	}

	private _calcPitch = function (quat, yaw) {
...
OrbitCamera.attributes.add('zoomOffset', {
	type: 'number',
	default: 0,
	title: 'Zoom Offset',
	description: 'Cameras default offset from the focused model',
});
